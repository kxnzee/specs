...
- адаптация интеграции jitsi-web с зависимостью lib-jitsi-meet (версия
  зафиксирована/pinned) для чтения этого значения, обновление типов, UI и тестов
  jitsi-web. Точный технический способ адаптации уточнён на Design — см. раздел 9.
...
- изменение покрыто тестами в jitsi-control (формирование ответа) и в jitsi-web
  (адаптация lib-jitsi-meet, типы, UI).

Проверка результата выполняется через автотесты в обоих репозиториях и визуальную
проверку строки в интерфейсе конференции при настроенном и при не настроенном
регионе на backend.
...
- `jitsi-control` — формирование ответа создания конференции с настроенным
  ...
  региона.
- `jitsi-web` — адаптация интеграции с зависимостью lib-jitsi-meet (версия
  зафиксирована/pinned) для чтения нового значения, обновление типов, UI
  (верхняя информационная строка конференции) и тестов. Причина: отображение
...
Repository-id соответствуют реестру проекта (`jitsi-control`, `jitsi-web`,
`jitsi-videobridge`, роль `code`). Точный состав репозиториев в Repository Impact
будет подтверждён на Proposal.
...
- Исходная формулировка задачи описывала интеграцию jitsi-web с lib-jitsi-meet как
  "зафиксированный форк через tracked patch", подразумевая существующий патч,
  который нужно адаптировать. Repository evidence (CodeGraph, репозиторий
  jitsi-web) на этапе Design подтвердило, что версия lib-jitsi-meet зафиксирована
  (pinned), но отдельного существующего патча именно для этой зависимости сейчас
  нет — механизм патчей зависимостей в репозитории применяется к другим
  библиотекам. Наблюдаемое поведение и scope Change это не меняет: адаптация
  интеграции jitsi-web с lib-jitsi-meet по-прежнему требуется, изменился только
  технический факт "патч уже существует" → "патч отсутствует, будет введён при
  реализации". Формулировки во всём документе и в Proposal приведены в
  соответствие с этим уточнением.
