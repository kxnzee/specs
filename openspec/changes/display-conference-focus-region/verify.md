...
- **Риски:** Кандидат ещё не записан Change Tracking, а визуальная проверка исключена
  из текущего прогона по решению пользователя.
...
- **Кандидат:** `openspec/changes/display-conference-focus-region/implementation-map.yaml`
  — будет создан Change Tracking после публикации code-кандидатов.
...
- **Кратко:** Требования трассируются к коду и автоматическим проверкам, но Change
  Tracking ещё не содержит опубликованный кандидат, а ручная UI-проверка отложена.
- **Evidence:** `proposal.md`, `design.md`, `tasks.md`, локальные результаты проверок
  трёх затронутых репозиториев.

## Открытые вопросы

- После публикации code-кандидатов записать их в Change Tracking.
