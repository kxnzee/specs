...
- [ ] 1.1 Добавить `focus-region` в ответ создания конференции при непустом
      `jicofo.local-region` ([SC-CONFERENCE-REGION-DIAGNOSTICS-001]).
- [ ] 1.2 Не добавлять свойство при отсутствующем или пустом значении
...
- [ ] 2.1 Разобрать непустое `focus-region` из ответа создания конференции и
      предоставить его через API конференции
      ([SC-CONFERENCE-REGION-DIAGNOSTICS-003],
      [SC-CONFERENCE-REGION-DIAGNOSTICS-004]).
- [ ] 2.2 Зафиксировать значение или его отсутствие по первому успешному ответу и
...
- [ ] 3.1 Добавить web-only label `Conference region: <value>` в существующую
      строку `ConferenceInfo` ([SC-CONFERENCE-REGION-DIAGNOSTICS-003]).
- [ ] 3.2 Скрывать label при отсутствующем, пустом или некорректном значении без
      ошибки для участника ([SC-CONFERENCE-REGION-DIAGNOSTICS-004]).
- [ ] 3.3 Получать подпись через i18n и передавать значение как обычный React text
      без интерактивности ([SC-CONFERENCE-REGION-DIAGNOSTICS-006]).
- [ ] 3.4 Не менять мобильные компоненты
