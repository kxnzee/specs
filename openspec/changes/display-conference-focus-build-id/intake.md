...
подтвердила источник значения: `CurrentVersionImpl.NIGHTLY_BUILD_ID` в Jicofo.
...
Используется `CurrentVersionImpl.NIGHTLY_BUILD_ID`. На этапе Design адресная
проверка кода подтвердила, что это значение уже передаётся всем участникам внутри
`CurrentVersionImpl.VERSION.toString()` в существующем presence-компоненте версии.
...
- Адресная проверка кода на Design подтвердила
  `CurrentVersionImpl.NIGHTLY_BUILD_ID` как источник build ID и его существующую
  передачу всем участникам внутри строки версии. Основание и пути приведены в
  `design.md`.
